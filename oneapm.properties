...
###############################################################################
# 默认会对所有的包都执行插码操作，属性 IncludePackages ExcludePackages 可以结合使用来确定
# 对指定的包插码，或者指定的包不插码
#
# 例如, IncludeAllPackages=true & ExcludePackages=a.b
# 会对所有的包插码，但是对包 a.b 不会插码
#
# 例如，IncludeAllPackages=false & IncludePackages=a.b
...
###############################################################################
# Http 数据包gzip压缩功能，默认为 false 发送数据使用gzip压缩功能，如您不需要，设置为true，
# gzip 压缩就不会生效。
...
###############################################################################
# 您可以在此处定义您的 appName，如果appName没有在此处定义，那么会使用定义在AndroidManifest.xml 中的 application
# name, 作为ApplicationID
...
# 配置so 库的时候主要要取所有so库 ABI 的最小子集，不然在某些cpu 架构上会报错
# 可选项有 : 'x86', 'x86_64', 'armeabi', 'armeabi-v7a', 'arm64-v8a'
# 多个ABI 之间用英文逗号隔开
